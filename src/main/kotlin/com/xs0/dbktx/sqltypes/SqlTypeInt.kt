...
class SqlTypeInt(private val concreteType: SqlTypeKind, isNotNull: Boolean, isAutoGenerated: Boolean, isUnsigned: Boolean) : SqlTypeNumeric<Int>(isNotNull, isAutoGenerated, isUnsigned) {
    ...
    override fun toSql(value: Int, sql: Sql) {
        if (isUnsigned && concreteType === SqlTypeKind.INT && value < 0) {
            sql(Integer.toUnsignedLong(value))
        } else {
            sql(value)
        }
    }
